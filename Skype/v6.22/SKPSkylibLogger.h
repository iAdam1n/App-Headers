/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:44:42 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/BCB8F06C-D639-4947-A2BF-415A0A66C023/Skype.app/Skype
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Skype/DDAbstractLogger.h>

@class NSMutableArray;

@interface SKPSkylibLogger : DDAbstractLogger {

	long long _loggingStatus;
	NSMutableArray* _queue;

}

@property (nonatomic,retain) NSMutableArray * queue;               //@synthesize queue=_queue - In the implementation block
@property (assign,nonatomic) long long loggingStatus;              //@synthesize loggingStatus=_loggingStatus - In the implementation block
-(void)processMessagesInQueue;
-(long long)loggingStatus;
-(void)setLoggingStatus:(long long)arg1 ;
-(id)init;
-(NSMutableArray *)queue;
-(void)setQueue:(NSMutableArray *)arg1 ;
-(void)logMessage:(id)arg1 ;
@end


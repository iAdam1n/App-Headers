/*
* This header is generated by classdump-dyld 0.7
* on Monday, November 2, 2015 at 10:04:53 AM Greenwich Mean Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /private/var/mobile/Containers/Bundle/Application/6EA2C334-4708-4A2A-9290-87A765CA9E42/WhatsApp.app/WhatsApp
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <WhatsApp/WAFieldStatsEvent.h>

@interface WAFieldStatsEventWebcWsOpen : WAFieldStatsEvent {

	double _webc_ws_open_t;

}

@property (assign,nonatomic) double webc_ws_open_t;              //@synthesize webc_ws_open_t=_webc_ws_open_t - In the implementation block
-(double)webc_ws_open_t;
-(void)setWebc_ws_open_t:(double)arg1 ;
-(id)init;
-(void)submit;
@end


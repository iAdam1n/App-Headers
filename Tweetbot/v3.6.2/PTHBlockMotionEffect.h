/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 1:06:47 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/ECBB6493-3CD3-4031-8AE5-B625F29152BF/Tweetbot.app/Tweetbot
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Tweetbot/Tweetbot-Structs.h>
#import <UIKit/UIMotionEffect.h>

@interface PTHBlockMotionEffect : UIMotionEffect {

	/*^block*/id _valuesBlock;

}

@property (nonatomic,copy) id valuesBlock;              //@synthesize valuesBlock=_valuesBlock - In the implementation block
-(id)valuesBlock;
-(void)setValuesBlock:(id)arg1 ;
-(id)keyPathsAndRelativeValuesForViewerOffset:(UIOffset)arg1 ;
@end


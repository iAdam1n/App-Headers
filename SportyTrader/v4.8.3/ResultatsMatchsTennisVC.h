/*
* This header is generated by classdump-dyld 1.0
* on Thursday, August 25, 2016 at 10:10:00 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/4C83B32D-E9D7-434B-B0B8-8A6C2C13527D/SportyTrader.app/SportyTrader
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SportyTrader/ResultatsMatchsVC.h>

@class TournamentWithMatches;

@interface ResultatsMatchsTennisVC : ResultatsMatchsVC {

	TournamentWithMatches* currentTournament;

}

@property (nonatomic,retain) TournamentWithMatches * currentTournament; 
-(id)getDateLastRequest;
-(void)updateLastRequestDate;
-(id)flurryTag;
-(id)URLStringForFullRequestWithDateMaths:(id)arg1 ;
-(id)URLStringForRefreshRequestWithLastUpdateDate:(id)arg1 ;
-(id)availableDatesKey;
-(id)URLStringForDatesRequest;
-(void)prepareTournamentsArray;
-(TournamentWithMatches *)currentTournament;
-(void)setCurrentTournament:(TournamentWithMatches *)arg1 ;
-(void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2 ;
-(id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2 ;
-(void)didReceiveMemoryWarning;
-(id)initWithNibName:(id)arg1 bundle:(id)arg2 ;
-(void)viewWillAppear:(BOOL)arg1 ;
-(void)viewDidLoad;
@end


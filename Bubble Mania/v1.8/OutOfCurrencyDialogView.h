/*
* This header is generated by classdump-dyld 1.0
* on Thursday, August 25, 2016 at 11:58:13 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/0CF72264-5488-4FD5-B700-EF786D177983/BubbleMania.app/BubbleMania
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <BubbleMania/XibDialogView.h>
#import <BubbleMania/StoreManagerDelegate.h>

@class UILabel, UIImageView, SKProduct, NSString;

@interface OutOfCurrencyDialogView : XibDialogView <StoreManagerDelegate> {

	UILabel* amount;
	UILabel* cost;
	UIImageView* saleTagImage;
	SKProduct* product;

}

@property (nonatomic,retain) SKProduct * product; 
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)viewForItem:(id)arg1 ;
+(id)viewForCost:(int)arg1 favorCost:(int)arg2 ;
-(void)dismissed:(id)arg1 ;
-(void)accepted:(id)arg1 ;
-(id)localizedPrice:(id)arg1 ;
-(void)processTransactionResponse:(id)arg1 ;
-(void)buyAppleStoreProduct:(id)arg1 ;
-(id)initWithCost:(int)arg1 ;
-(id)productInfo;
-(void)updateProductForCost:(id)arg1 cost:(int)arg2 ;
-(void)dealloc;
-(void)setProduct:(SKProduct *)arg1 ;
-(SKProduct *)product;
@end


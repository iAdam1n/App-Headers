/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:39:33 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/C3C2BD9F-C9D4-4E48-9EC0-40E7F1350599/Facebook.app/Facebook
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Facebook/Facebook-Structs.h>
#import <Facebook/CKCompositeComponent.h>

@class FBMemFeedStory, FBFeedEventBus, FBFeedToolbox;

@interface FBFeedStoryStyledUFIComponent : CKCompositeComponent {

	FBMemFeedStory* _story;
	SEL _likeAction;
	FBFeedEventBus* _eventBus;
	FBFeedToolbox* _toolbox;

}
+(id)newWithStory:(id)arg1 likeAction:(SEL)arg2 commentAction:(SEL)arg3 shareAction:(SEL)arg4 countsAction:(SEL)arg5 inlineVoiceSwitcherAction:(SEL)arg6 options:(const FBFeedStoryStyledUFIComponentOptions*)arg7 toolbox:(id)arg8 ;
-(void)viewerDidLike:(id)arg1 context:(id)arg2 ;
-(void)viewerDidReact:(id)arg1 context:(id)arg2 ;
@end


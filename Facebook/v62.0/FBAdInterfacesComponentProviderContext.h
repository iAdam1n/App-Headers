/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:39:32 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/C3C2BD9F-C9D4-4E48-9EC0-40E7F1350599/Facebook.app/Facebook
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class FBAdInterfacesPromotionViewController, FBAdInterfacesPromotionDataModel;

@interface FBAdInterfacesComponentProviderContext : NSObject {

	FBAdInterfacesPromotionViewController* _viewController;

}

@property (nonatomic,readonly) FBAdInterfacesPromotionDataModel * dataModel; 
@property (nonatomic,__weak,readonly) FBAdInterfacesPromotionViewController * viewController;              //@synthesize viewController=_viewController - In the implementation block
-(id)story;
-(BOOL)allowInstagramAdPreview;
-(id)toolbox;
-(id)feedComponentToolbox;
-(FBAdInterfacesPromotionDataModel *)dataModel;
-(id)initWithPromotionViewController:(id)arg1 ;
-(FBAdInterfacesPromotionViewController *)viewController;
@end


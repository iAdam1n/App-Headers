/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:39:39 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/C3C2BD9F-C9D4-4E48-9EC0-40E7F1350599/Facebook.app/Facebook
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <UIKit/UIView.h>
#import <Facebook/CKComponentHostingViewDelegate.h>

@class CKComponentHostingView, UIView, UICollectionView, FBMemScreenAction, NSString;

@interface FBPageComponentFlowMainView : UIView <CKComponentHostingViewDelegate> {

	CKComponentHostingView* _headerHostingView;
	UIView* _headerSeparatorView;
	CKComponentHostingView* _footerHostingView;
	UIView* _footerSeparatorView;
	CKComponentHostingView* _actionHostingView;
	CKComponentHostingView* _disclaimerHostingView;
	UICollectionView* _collectionView;
	FBMemScreenAction* _screenAction;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)componentHostingViewDidInvalidateSize:(id)arg1 ;
-(void)configureWithScreenData:(id)arg1 context:(id)arg2 ;
-(void)updateScreenActionWithStatus:(BOOL)arg1 context:(id)arg2 ;
-(void)layoutSubviews;
-(id)initWithCollectionView:(id)arg1 ;
@end


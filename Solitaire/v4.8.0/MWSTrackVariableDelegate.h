/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 12:03:42 AM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/FE008365-307B-4CDF-8948-1B974F28A799/SolitaireFree.app/SolitaireFree
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@protocol MWSTrackVariableDelegate <NSObject>
@optional
-(id)getUpdatedVariableFromAppForData:(id)arg1;
-(void)updateVariableFailed:(id)arg1 forIdentifier:(id)arg2;
-(void)configHandleInitialLoadOfVariables;
-(void)configHandleUpdateOfVariablesWithType:(int)arg1;
-(BOOL)configIsVariableAllowedUnlimitedUpdates:(unsigned long long)arg1;

@required
-(id)configGetAllVariableNames;
-(id)configGetAllVariableParamNames;
-(int)configVariableToType:(unsigned long long)arg1;
-(id)configVariableToDefault:(unsigned long long)arg1;
-(int)configVariableParamToType:(unsigned long long)arg1;
-(id)configVariableOfflineKey;

@end


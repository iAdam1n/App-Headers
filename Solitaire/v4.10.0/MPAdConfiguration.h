/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:42:14 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/2E0DF6B4-8C57-4F66-9862-2B2C129D4027/SolitaireFree.app/SolitaireFree
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


#import <SolitaireFree/SolitaireFree-Structs.h>
@class NSString, NSURL, NSData, NSDictionary, NSDate, MPRewardedVideoReward;

@interface MPAdConfiguration : NSObject {

	BOOL _adUnitWarmingUp;
	BOOL _shouldInterceptLinks;
	BOOL _scrollable;
	BOOL _precacheRequired;
	BOOL _isVastVideoPlayer;
	unsigned long long _adType;
	NSString* _networkType;
	NSURL* _clickTrackingURL;
	NSURL* _impressionTrackingURL;
	NSURL* _failoverURL;
	NSURL* _interceptURLPrefix;
	double _refreshInterval;
	double _adTimeoutInterval;
	NSData* _adResponseData;
	NSDictionary* _nativeSDKParameters;
	NSString* _customSelectorName;
	Class _customEventClass;
	NSDictionary* _customEventClassData;
	unsigned long long _orientationType;
	NSString* _dspCreativeId;
	NSDate* _creationTimestamp;
	NSString* _creativeId;
	NSString* _headerAdType;
	long long _nativeVideoPlayVisiblePercent;
	long long _nativeVideoPauseVisiblePercent;
	long long _nativeVideoImpressionMinVisiblePercent;
	double _nativeVideoImpressionVisible;
	double _nativeVideoMaxBufferingTime;
	MPRewardedVideoReward* _rewardedVideoReward;
	long long _skipTime;
	NSURL* _expand;
	NSURL* _collapse;
	NSURL* _start;
	NSURL* _firstQuartile;
	NSURL* _midPoint;
	NSURL* _thirdQuartile;
	NSURL* _complete;
	NSString* _adResponseHTMLString;
	CGSize _preferredSize;

}

@property (nonatomic,copy) NSString * adResponseHTMLString;                                 //@synthesize adResponseHTMLString=_adResponseHTMLString - In the implementation block
@property (assign,nonatomic) unsigned long long adType;                                     //@synthesize adType=_adType - In the implementation block
@property (assign,nonatomic) BOOL adUnitWarmingUp;                                          //@synthesize adUnitWarmingUp=_adUnitWarmingUp - In the implementation block
@property (nonatomic,copy) NSString * networkType;                                          //@synthesize networkType=_networkType - In the implementation block
@property (assign,nonatomic) CGSize preferredSize;                                          //@synthesize preferredSize=_preferredSize - In the implementation block
@property (nonatomic,retain) NSURL * clickTrackingURL;                                      //@synthesize clickTrackingURL=_clickTrackingURL - In the implementation block
@property (nonatomic,retain) NSURL * impressionTrackingURL;                                 //@synthesize impressionTrackingURL=_impressionTrackingURL - In the implementation block
@property (nonatomic,retain) NSURL * failoverURL;                                           //@synthesize failoverURL=_failoverURL - In the implementation block
@property (nonatomic,retain) NSURL * interceptURLPrefix;                                    //@synthesize interceptURLPrefix=_interceptURLPrefix - In the implementation block
@property (assign,nonatomic) BOOL shouldInterceptLinks;                                     //@synthesize shouldInterceptLinks=_shouldInterceptLinks - In the implementation block
@property (assign,nonatomic) BOOL scrollable;                                               //@synthesize scrollable=_scrollable - In the implementation block
@property (assign,nonatomic) double refreshInterval;                                        //@synthesize refreshInterval=_refreshInterval - In the implementation block
@property (assign,nonatomic) double adTimeoutInterval;                                      //@synthesize adTimeoutInterval=_adTimeoutInterval - In the implementation block
@property (nonatomic,copy) NSData * adResponseData;                                         //@synthesize adResponseData=_adResponseData - In the implementation block
@property (nonatomic,retain) NSDictionary * nativeSDKParameters;                            //@synthesize nativeSDKParameters=_nativeSDKParameters - In the implementation block
@property (nonatomic,copy) NSString * customSelectorName;                                   //@synthesize customSelectorName=_customSelectorName - In the implementation block
@property (assign,nonatomic) Class customEventClass;                                        //@synthesize customEventClass=_customEventClass - In the implementation block
@property (nonatomic,retain) NSDictionary * customEventClassData;                           //@synthesize customEventClassData=_customEventClassData - In the implementation block
@property (assign,nonatomic) unsigned long long orientationType;                            //@synthesize orientationType=_orientationType - In the implementation block
@property (nonatomic,copy) NSString * dspCreativeId;                                        //@synthesize dspCreativeId=_dspCreativeId - In the implementation block
@property (assign,nonatomic) BOOL precacheRequired;                                         //@synthesize precacheRequired=_precacheRequired - In the implementation block
@property (assign,nonatomic) BOOL isVastVideoPlayer;                                        //@synthesize isVastVideoPlayer=_isVastVideoPlayer - In the implementation block
@property (nonatomic,retain) NSDate * creationTimestamp;                                    //@synthesize creationTimestamp=_creationTimestamp - In the implementation block
@property (nonatomic,copy) NSString * creativeId;                                           //@synthesize creativeId=_creativeId - In the implementation block
@property (nonatomic,copy) NSString * headerAdType;                                         //@synthesize headerAdType=_headerAdType - In the implementation block
@property (assign,nonatomic) long long nativeVideoPlayVisiblePercent;                       //@synthesize nativeVideoPlayVisiblePercent=_nativeVideoPlayVisiblePercent - In the implementation block
@property (assign,nonatomic) long long nativeVideoPauseVisiblePercent;                      //@synthesize nativeVideoPauseVisiblePercent=_nativeVideoPauseVisiblePercent - In the implementation block
@property (assign,nonatomic) long long nativeVideoImpressionMinVisiblePercent;              //@synthesize nativeVideoImpressionMinVisiblePercent=_nativeVideoImpressionMinVisiblePercent - In the implementation block
@property (assign,nonatomic) double nativeVideoImpressionVisible;                           //@synthesize nativeVideoImpressionVisible=_nativeVideoImpressionVisible - In the implementation block
@property (assign,nonatomic) double nativeVideoMaxBufferingTime;                            //@synthesize nativeVideoMaxBufferingTime=_nativeVideoMaxBufferingTime - In the implementation block
@property (nonatomic,retain) MPRewardedVideoReward * rewardedVideoReward;                   //@synthesize rewardedVideoReward=_rewardedVideoReward - In the implementation block
@property (assign,nonatomic) long long skipTime;                                            //@synthesize skipTime=_skipTime - In the implementation block
@property (nonatomic,retain) NSURL * expand;                                                //@synthesize expand=_expand - In the implementation block
@property (nonatomic,retain) NSURL * collapse;                                              //@synthesize collapse=_collapse - In the implementation block
@property (nonatomic,retain) NSURL * start;                                                 //@synthesize start=_start - In the implementation block
@property (nonatomic,retain) NSURL * firstQuartile;                                         //@synthesize firstQuartile=_firstQuartile - In the implementation block
@property (nonatomic,retain) NSURL * midPoint;                                              //@synthesize midPoint=_midPoint - In the implementation block
@property (nonatomic,retain) NSURL * thirdQuartile;                                         //@synthesize thirdQuartile=_thirdQuartile - In the implementation block
@property (nonatomic,retain) NSURL * complete;                                              //@synthesize complete=_complete - In the implementation block
+(id)getHostName;
+(void)setHostName:(id)arg1 ;
-(NSURL *)firstQuartile;
-(NSURL *)thirdQuartile;
-(NSURL *)collapse;
-(void)setFirstQuartile:(NSURL *)arg1 ;
-(void)setThirdQuartile:(NSURL *)arg1 ;
-(void)setCollapse:(NSURL *)arg1 ;
-(void)setAdType:(unsigned long long)arg1 ;
-(Class)customEventClass;
-(NSDictionary *)customEventClassData;
-(double)adTimeoutInterval;
-(MPRewardedVideoReward *)rewardedVideoReward;
-(NSString *)dspCreativeId;
-(BOOL)hasPreferredSize;
-(NSData *)adResponseData;
-(unsigned long long)orientationType;
-(void)setOrientationType:(unsigned long long)arg1 ;
-(BOOL)adUnitWarmingUp;
-(NSURL *)failoverURL;
-(void)setImpressionTrackingURL:(NSURL *)arg1 ;
-(void)setClickTrackingURL:(NSURL *)arg1 ;
-(NSString *)adResponseHTMLString;
-(long long)skipTime;
-(NSString *)headerAdType;
-(void)setDspCreativeId:(NSString *)arg1 ;
-(BOOL)scrollable;
-(BOOL)shouldInterceptLinks;
-(id)clickDetectionURLPrefix;
-(NSURL *)impressionTrackingURL;
-(NSURL *)clickTrackingURL;
-(void)setAdResponseData:(NSData *)arg1 ;
-(unsigned long long)adTypeFromHeaders:(id)arg1 ;
-(void)setAdUnitWarmingUp:(BOOL)arg1 ;
-(id)networkTypeFromHeaders:(id)arg1 ;
-(id)URLFromHeaders:(id)arg1 forKey:(id)arg2 ;
-(void)setFailoverURL:(NSURL *)arg1 ;
-(void)setInterceptURLPrefix:(NSURL *)arg1 ;
-(void)setShouldInterceptLinks:(BOOL)arg1 ;
-(void)setScrollable:(BOOL)arg1 ;
-(double)refreshIntervalFromHeaders:(id)arg1 ;
-(double)adTimeoutIntervalFromHeaders:(id)arg1 ;
-(void)setAdTimeoutInterval:(double)arg1 ;
-(id)dictionaryFromHeaders:(id)arg1 forKey:(id)arg2 ;
-(void)setNativeSDKParameters:(NSDictionary *)arg1 ;
-(void)setCustomSelectorName:(NSString *)arg1 ;
-(unsigned long long)orientationTypeFromHeaders:(id)arg1 ;
-(Class)setUpCustomEventClassFromHeaders:(id)arg1 ;
-(void)setCustomEventClass:(Class)arg1 ;
-(id)customEventClassDataFromHeaders:(id)arg1 ;
-(void)setPrecacheRequired:(BOOL)arg1 ;
-(void)setIsVastVideoPlayer:(BOOL)arg1 ;
-(void)setHeaderAdType:(NSString *)arg1 ;
-(void)setSkipTime:(long long)arg1 ;
-(void)setExpand:(NSURL *)arg1 ;
-(long long)percentFromHeaders:(id)arg1 forKey:(id)arg2 ;
-(void)setNativeVideoPlayVisiblePercent:(long long)arg1 ;
-(void)setNativeVideoPauseVisiblePercent:(long long)arg1 ;
-(void)setNativeVideoImpressionMinVisiblePercent:(long long)arg1 ;
-(double)timeIntervalFromMsHeaders:(id)arg1 forKey:(id)arg2 ;
-(void)setNativeVideoImpressionVisible:(double)arg1 ;
-(void)setNativeVideoMaxBufferingTime:(double)arg1 ;
-(id)adAmountFromHeaders:(id)arg1 key:(id)arg2 ;
-(void)setRewardedVideoReward:(MPRewardedVideoReward *)arg1 ;
-(void)setAdResponseHTMLString:(NSString *)arg1 ;
-(NSURL *)interceptURLPrefix;
-(NSDictionary *)nativeSDKParameters;
-(NSString *)customSelectorName;
-(id)initWithHeaders:(id)arg1 data:(id)arg2 ;
-(long long)nativeVideoPlayVisiblePercent;
-(long long)nativeVideoPauseVisiblePercent;
-(long long)nativeVideoImpressionMinVisiblePercent;
-(double)nativeVideoImpressionVisible;
-(double)nativeVideoMaxBufferingTime;
-(void)setCustomEventClassData:(NSDictionary *)arg1 ;
-(BOOL)precacheRequired;
-(BOOL)isVastVideoPlayer;
-(NSURL *)start;
-(void)setStart:(NSURL *)arg1 ;
-(NSURL *)complete;
-(CGSize)preferredSize;
-(void)setComplete:(NSURL *)arg1 ;
-(NSURL *)expand;
-(void)setNetworkType:(NSString *)arg1 ;
-(double)refreshInterval;
-(NSString *)networkType;
-(unsigned long long)adType;
-(void)setRefreshInterval:(double)arg1 ;
-(NSURL *)midPoint;
-(void)setMidPoint:(NSURL *)arg1 ;
-(void)setCreationTimestamp:(NSDate *)arg1 ;
-(NSDate *)creationTimestamp;
-(void)setCreativeId:(NSString *)arg1 ;
-(NSString *)creativeId;
-(void)setPreferredSize:(CGSize)arg1 ;
@end


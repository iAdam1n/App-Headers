/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:42:14 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/2E0DF6B4-8C57-4F66-9862-2B2C129D4027/SolitaireFree.app/SolitaireFree
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SolitaireFree/VungleSerialQueueProvider.h>

@protocol OS_dispatch_queue;
@class NSObject, NSString;

@interface VungleFileManagerQueueProvider : NSObject <VungleSerialQueueProvider> {

	NSObject*<OS_dispatch_queue> _fileManagerQueue;

}

@property (retain) NSObject*<OS_dispatch_queue> fileManagerQueue;              //@synthesize fileManagerQueue=_fileManagerQueue - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(id)sharedProvider;
-(id)provideSerialQueue;
-(NSObject*<OS_dispatch_queue>)fileManagerQueue;
-(void)setFileManagerQueue:(NSObject*<OS_dispatch_queue>)arg1 ;
-(void)setupSerialQueue;
-(id)init;
@end


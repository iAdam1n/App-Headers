/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 12:10:13 PM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/EECA69E8-8DA6-473D-BDCA-9D4EFB22C6CE/Instagram.app/Instagram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Instagram/IGDirectContentUpload.h>
#import <Instagram/NSCoding.h>

@class NSData, NSNumber, NSString;

@interface IGDirectContentVideoUpload : IGDirectContentUpload <NSCoding> {

	NSData* _videoData;
	NSNumber* _uploadID;
	NSString* _videoResult;
	long long _devicePosition;

}

@property (nonatomic,retain) NSData * videoData;                    //@synthesize videoData=_videoData - In the implementation block
@property (nonatomic,retain) NSNumber * uploadID;                   //@synthesize uploadID=_uploadID - In the implementation block
@property (nonatomic,copy) NSString * videoResult;                  //@synthesize videoResult=_videoResult - In the implementation block
@property (assign,nonatomic) long long devicePosition;              //@synthesize devicePosition=_devicePosition - In the implementation block
-(long long)devicePosition;
-(void)setVideoResult:(NSString *)arg1 ;
-(void)setDevicePosition:(long long)arg1 ;
-(void)setVideoData:(NSData *)arg1 ;
-(NSData *)videoData;
-(NSNumber *)uploadID;
-(void)setUploadID:(NSNumber *)arg1 ;
-(id)initWithCoder:(id)arg1 ;
-(void)encodeWithCoder:(id)arg1 ;
-(id)description;
-(NSString *)videoResult;
@end


/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 11:30:48 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/16C752D5-8F70-4E83-B0AB-94FF56CA8C26/Twitter.app/Twitter
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Twitter/TFNAssetsLibraryPhotoObject.h>
#import <Twitter/TFNPhotoAsset.h>

@class NSDate, CLLocation, NSString, UIImage, ALAssetRepresentation, NSURL;

@interface TFNAssetsLibraryPhotoAsset : TFNAssetsLibraryPhotoObject <TFNPhotoAsset>

@property (nonatomic,readonly) UIImage * assetThumbnail; 
@property (nonatomic,readonly) ALAssetRepresentation * assetRepresentation; 
@property (nonatomic,readonly) UIImage * sourceImage; 
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (nonatomic,readonly) NSString * localIdentifier; 
@property (nonatomic,readonly) NSURL * assetsLibraryURL; 
@property (nonatomic,readonly) long long mediaType; 
@property (nonatomic,readonly) unsigned long long mediaSubtypes; 
@property (nonatomic,readonly) unsigned long long pixelWidth; 
@property (nonatomic,readonly) unsigned long long pixelHeight; 
@property (nonatomic,readonly) NSDate * creationDate; 
@property (nonatomic,readonly) NSDate * modificationDate; 
@property (nonatomic,readonly) CLLocation * location; 
@property (nonatomic,readonly) double duration; 
@property (getter=isHidden,nonatomic,readonly) BOOL hidden; 
@property (getter=isFavorite,nonatomic,readonly) BOOL favorite; 
@property (nonatomic,readonly) NSString * burstIdentifier; 
@property (nonatomic,readonly) unsigned long long burstSelectionTypes; 
@property (nonatomic,readonly) BOOL representsBurst; 
@property (getter=isMediaSubtypesAvailable,nonatomic,readonly) BOOL mediaSubtypesAvailable; 
+(id)fetchAssetsWithMediaType:(long long)arg1 options:(id)arg2 ;
+(id)fetchAssetsWithLocalIdentifiers:(id)arg1 options:(id)arg2 ;
+(id)fetchAssetsInAssetCollection:(id)arg1 options:(id)arg2 ;
+(id)fetchKeyAssetsInAssetCollection:(id)arg1 options:(id)arg2 ;
+(id)fetchAssetsWithOptions:(id)arg1 ;
+(id)fetchAssetsWithALAssetURLs:(id)arg1 options:(id)arg2 ;
-(BOOL)isMediaSubtypesAvailable;
-(UIImage *)assetThumbnail;
-(ALAssetRepresentation *)assetRepresentation;
-(unsigned long long)burstSelectionTypes;
-(BOOL)representsBurst;
-(unsigned long long)pixelWidth;
-(unsigned long long)pixelHeight;
-(unsigned long long)mediaSubtypes;
-(BOOL)isHidden;
-(double)duration;
-(NSDate *)modificationDate;
-(CLLocation *)location;
-(NSDate *)creationDate;
-(UIImage *)sourceImage;
-(BOOL)canPerformEditOperation:(long long)arg1 ;
-(NSString *)burstIdentifier;
-(BOOL)isFavorite;
-(long long)mediaType;
@end


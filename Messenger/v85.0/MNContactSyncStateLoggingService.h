/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:18:18 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/6F5201FD-3D2F-4BE1-80E5-E1251C7564E3/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/MNServiceControllable.h>
#import <Messenger/FBClassProvidable.h>

@protocol FBAddressBookAccessPermissionChecker;
@class FBMobileConfigContextManager, MNUserSettings, FBAnalytics, NSString;

@interface MNContactSyncStateLoggingService : NSObject <MNServiceControllable, FBClassProvidable> {

	id<FBAddressBookAccessPermissionChecker> _addressBookAccessPermissionChecker;
	FBMobileConfigContextManager* _configManager;
	MNUserSettings* _userSettings;
	FBAnalytics* _analytics;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id)initWithProviderMapData:(id)arg1 ;
-(id)initWithAddressBookAccessPermissionChecker:(id)arg1 configManager:(id)arg2 userSettings:(id)arg3 analytics:(id)arg4 ;
-(void)_removeNotificationObserver;
-(void)_addNotificationObserver;
-(BOOL)_shouldLogContactSyncState;
-(void)_logContactSyncState;
-(void)_setContactSyncStateLastLogDate;
-(void)dealloc;
-(void)stop;
-(void)_applicationWillResignActive:(id)arg1 ;
-(void)start:(id)arg1 ;
-(void)handleIdle;
@end


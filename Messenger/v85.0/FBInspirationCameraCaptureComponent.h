/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:18:23 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/6F5201FD-3D2F-4BE1-80E5-E1251C7564E3/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/Messenger-Structs.h>
#import <Messenger/CKCompositeComponent.h>

@class CKComponent, FBCameraComponentToolbox;

@interface FBInspirationCameraCaptureComponent : CKCompositeComponent {

	unsigned long long _captureState;
	long long _cameraState;
	CKComponent* _buttonComponent;
	CKComponent* _scaleButtonComponent;
	BOOL _disableCaptureButton;
	FBCameraComponentToolbox* _toolbox;

}
+(id)newWithCaptureState:(unsigned long long)arg1 disableCaptureButton:(BOOL)arg2 cameraState:(long long)arg3 toolbox:(id)arg4 ;
-(vector<CKComponentAnimation, std::__1::allocator<CKComponentAnimation> >*)animationsFromPreviousComponent:(id)arg1 ;
@end


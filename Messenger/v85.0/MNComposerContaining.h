/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:18:21 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/6F5201FD-3D2F-4BE1-80E5-E1251C7564E3/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

@class UIView;


@protocol MNComposerContaining <MNActionNuxPresenter,MNComposerBannerViewPresenter,MNPreviewViewPresenter>
@property (nonatomic,retain) UIView * composerSearchBar; 
@property (nonatomic,retain) UIView * contentView; 
@property (assign,nonatomic,__weak) UIView * controlsView; 
@property (assign,nonatomic) long long layoutMode; 
@property (assign,getter=isComposeViewHidden,nonatomic) BOOL composeViewHidden; 
@property (assign,getter=isComposerSearchBarHidden,nonatomic) BOOL composerSearchBarHidden; 
@required
-(void)setLayoutMode:(long long)arg1;
-(BOOL)isComposerSearchBarHidden;
-(BOOL)isComposeViewHidden;
-(void)setComposerSearchBar:(id)arg1;
-(void)setComposeViewHidden:(BOOL)arg1 animated:(BOOL)arg2;
-(void)setControlsView:(id)arg1;
-(void)setComposerSearchBarHidden:(BOOL)arg1 animated:(BOOL)arg2;
-(void)setComposeViewHidden:(BOOL)arg1;
-(void)setComposerSearchBarHidden:(BOOL)arg1;
-(UIView *)composerSearchBar;
-(UIView *)controlsView;
-(UIView *)contentView;
-(void)setContentView:(id)arg1;
-(long long)layoutMode;

@end


/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 11:40:15 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/E282ADAB-AF96-4184-9890-B5E58B4BDDD0/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/


@class NSDate;

@interface FBGraphQLFetcherCacheLookupResponse : NSObject {

	BOOL _willDownloadFreshResponse;
	id _item;
	NSDate* _downloadDate;

}

@property (nonatomic,readonly) id item;                                     //@synthesize item=_item - In the implementation block
@property (nonatomic,readonly) NSDate * downloadDate;                       //@synthesize downloadDate=_downloadDate - In the implementation block
@property (nonatomic,readonly) BOOL willDownloadFreshResponse;              //@synthesize willDownloadFreshResponse=_willDownloadFreshResponse - In the implementation block
-(id)initWithItem:(id)arg1 downloadDate:(id)arg2 willDownloadFreshResponse:(BOOL)arg3 ;
-(NSDate *)downloadDate;
-(BOOL)willDownloadFreshResponse;
-(id)item;
@end


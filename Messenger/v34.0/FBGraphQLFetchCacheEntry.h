/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 11:40:15 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/E282ADAB-AF96-4184-9890-B5E58B4BDDD0/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/


@protocol FBGraphQLFetchUnit;
@interface FBGraphQLFetchCacheEntry : NSObject {

	BOOL _stale;
	id _item;
	id<FBGraphQLFetchUnit> _fetchUnit;

}

@property (nonatomic,readonly) id item;                                       //@synthesize item=_item - In the implementation block
@property (nonatomic,readonly) id<FBGraphQLFetchUnit> fetchUnit;              //@synthesize fetchUnit=_fetchUnit - In the implementation block
@property (nonatomic,readonly) BOOL stale;                                    //@synthesize stale=_stale - In the implementation block
-(id)initWithItem:(id)arg1 fetchUnit:(id)arg2 stale:(BOOL)arg3 ;
-(id<FBGraphQLFetchUnit>)fetchUnit;
-(BOOL)stale;
-(id)description;
-(id)item;
@end


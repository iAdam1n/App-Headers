/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 11:40:33 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/E282ADAB-AF96-4184-9890-B5E58B4BDDD0/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Messenger/FBVideoAutoPlayRule.h>

@class NSString, NSURL, FBReachabilityAnnouncer, FBClientProxy;

@interface FBVideoNetworkConnectionAutoPlayRule : FBVideoAutoPlayRule {

	NSString* _videoID;
	NSURL* _videoURL;
	BOOL _checkCache;
	long long _definition;
	unsigned long long _minBandwithSate;
	FBReachabilityAnnouncer* _reachabilityAnnouncer;
	FBClientProxy* _clientProxy;

}
-(id)reasonForErrorCode:(unsigned long long)arg1 ;
-(id)initWithClientProxy:(id)arg1 videoID:(id)arg2 reachabilityAnnouncer:(id)arg3 videoURL:(id)arg4 checkCache:(BOOL)arg5 definition:(long long)arg6 minBandwithSate:(unsigned long long)arg7 ;
-(unsigned long long)shouldAutoplay;
-(id)init;
@end


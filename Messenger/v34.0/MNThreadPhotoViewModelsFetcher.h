/*
* This header is generated by classdump-dyld 0.7
* on Friday, August 21, 2015 at 11:40:42 AM British Summer Time
* Operating System: Version 8.4 (Build 12H143)
* Image Source: /private/var/mobile/Containers/Bundle/Application/E282ADAB-AF96-4184-9890-B5E58B4BDDD0/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Messenger/FBClassProvidable.h>

@protocol FBProvider;
@class MNThreadPhotosFetchGraphQLRunner, MNThreadMediaFetcherCursorStore, FBMThreadKey, NSString;

@interface MNThreadPhotoViewModelsFetcher : NSObject <FBClassProvidable> {

	id<FBProvider> _photosFetchRunnerProvider;
	MNThreadPhotosFetchGraphQLRunner* _photosFetchRunner;
	MNThreadMediaFetcherCursorStore* _cursorStore;
	FBMThreadKey* _threadKey;
	BOOL _fetchedAllPhotos;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(id)initWithProviderMapData:(id)arg1 ;
-(void)_processFailureWithError:(id)arg1 failureBlock:(/*^block*/id)arg2 ;
-(void)configureWithThreadKey:(id)arg1 ;
-(id)initWithPhotoViewModelFetcherProvider:(id)arg1 cursorStore:(id)arg2 ;
-(void)_fetchPhotoViewModelsFromCursor:(id)arg1 successBlock:(/*^block*/id)arg2 failureBlock:(/*^block*/id)arg3 ;
-(void)_processSuccessWithMessageImagesConnection:(id)arg1 successBlock:(/*^block*/id)arg2 ;
-(void)fetchPhotoViewModelsFromBeginningWithSuccessBlock:(/*^block*/id)arg1 failureBlock:(/*^block*/id)arg2 ;
-(void)fetchPhotoViewModelsFromLastCursorSuccessBlock:(/*^block*/id)arg1 failureBlock:(/*^block*/id)arg2 ;
-(BOOL)hasFetchedPhotosForThreadKey:(id)arg1 ;
-(void)cancel;
-(void)dealloc;
@end


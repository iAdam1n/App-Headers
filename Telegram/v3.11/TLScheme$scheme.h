/*
* This header is generated by classdump-dyld 1.0
* on Friday, August 26, 2016 at 5:53:37 PM British Summer Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /var/containers/Bundle/Application/D5AA3D7F-F68B-43D8-8CC1-85DAFE886350/Telegram.app/Telegram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Telegram/Telegram-Structs.h>
#import <Telegram/TLScheme.h>

@class NSString, NSArray;

@interface TLScheme$scheme : TLScheme {

	int _version;
	NSString* _scheme_raw;
	NSArray* _types;
	NSArray* _methods;

}

@property (nonatomic,retain) NSString * scheme_raw;              //@synthesize scheme_raw=_scheme_raw - In the implementation block
@property (nonatomic,retain) NSArray * types;                    //@synthesize types=_types - In the implementation block
@property (nonatomic,retain) NSArray * methods;                  //@synthesize methods=_methods - In the implementation block
@property (assign,nonatomic) int version;                        //@synthesize version=_version - In the implementation block
-(int)TLconstructorSignature;
-(int)TLconstructorName;
-(id)TLbuildFromMetaObject:(shared_ptr<TLMetaObject>*)arg1 ;
-(void)TLfillFieldsWithValues:(map<int, TLConstructedValue, std::less<int>, std::allocator<std::pair<const int, TLConstructedValue> > >=_Rb_tree<int, std::pair<const int, TLConstructedValue>, std::_Select1st<std::pair<const int, TLConstructedValue> >, std::less<int>, std::allocator<std::pair<const int, TLConstructedValue> > >=_Rb_tree_impl<std::less<int>, false>=less<int>_Rb_tree_node_base=i_Rb_tree_node_base}_Rb_tree_node_base}_Rb_tree_node_base}}Q}}}Ref)arg1 ;
-(void)setScheme_raw:(NSString *)arg1 ;
-(void)setMethods:(NSArray *)arg1 ;
-(NSString *)scheme_raw;
-(NSArray *)methods;
-(NSArray *)types;
-(void)setVersion:(int)arg1 ;
-(int)version;
-(void)setTypes:(NSArray *)arg1 ;
@end


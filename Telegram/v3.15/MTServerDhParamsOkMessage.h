/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:46:35 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/D20B482D-7A4B-420E-BEFA-D4B2B5C94752/Telegram.app/Telegram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Telegram/MTServerDhParamsMessage.h>

@class NSData;

@interface MTServerDhParamsOkMessage : MTServerDhParamsMessage {

	NSData* _encryptedResponse;

}

@property (nonatomic,readonly) NSData * encryptedResponse;              //@synthesize encryptedResponse=_encryptedResponse - In the implementation block
-(NSData *)encryptedResponse;
-(id)initWithNonce:(id)arg1 serverNonce:(id)arg2 encryptedResponse:(id)arg3 ;
@end


/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:46:32 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/D20B482D-7A4B-420E-BEFA-D4B2B5C94752/Telegram.app/Telegram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Telegram/Telegram-Structs.h>
#import <Telegram/TLMetaRpc.h>

@class TLInputChannel, TLInputUser, TLChannelParticipantRole;

@interface TLRPCchannels_editAdmin : TLMetaRpc {

	TLInputChannel* _channel;
	TLInputUser* _user_id;
	TLChannelParticipantRole* _role;

}

@property (nonatomic,retain) TLInputChannel * channel;                     //@synthesize channel=_channel - In the implementation block
@property (nonatomic,retain) TLInputUser * user_id;                        //@synthesize user_id=_user_id - In the implementation block
@property (nonatomic,retain) TLChannelParticipantRole * role;              //@synthesize role=_role - In the implementation block
-(int)TLconstructorSignature;
-(int)TLconstructorName;
-(id)TLbuildFromMetaObject:(shared_ptr<TLMetaObject>*)arg1 ;
-(void)TLfillFieldsWithValues:(map<int, TLConstructedValue, std::less<int>, std::allocator<std::pair<const int, TLConstructedValue> > >Ref)arg1 ;
-(int)impliedResponseSignature;
-(int)layerVersion;
-(void)setUser_id:(TLInputUser *)arg1 ;
-(void)setChannel:(TLInputChannel *)arg1 ;
-(TLInputChannel *)channel;
-(Class)responseClass;
-(void)setRole:(TLChannelParticipantRole *)arg1 ;
-(TLChannelParticipantRole *)role;
-(TLInputUser *)user_id;
@end


/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:46:35 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/D20B482D-7A4B-420E-BEFA-D4B2B5C94752/Telegram.app/Telegram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Telegram/TGCollectionMenuController.h>

@protocol SDisposable;
@class NSArray, TGCollectionMenuSection, UIActivityIndicatorView, SMetaDisposable;

@interface TGGroupsInCommonController : TGCollectionMenuController {

	id<SDisposable> _updatedGroupsInCommonDisposable;
	NSArray* _conversations;
	TGCollectionMenuSection* _conversationsSection;
	UIActivityIndicatorView* _activityIndicator;
	SMetaDisposable* _navigateDisposable;

}
-(id)initWithUserId:(int)arg1 ;
-(void)setConversations:(id)arg1 ;
-(void)navigateToPeerId:(long long)arg1 cachedConversation:(id)arg2 ;
-(id)preloadedConversationWithCachedConversation:(id)arg1 ;
-(void)dealloc;
-(void)viewDidLoad;
@end


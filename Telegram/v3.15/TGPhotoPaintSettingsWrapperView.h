/*
* This header is generated by classdump-dyld 1.0
* on Saturday, December 24, 2016 at 12:46:34 AM Greenwich Mean Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /var/containers/Bundle/Application/D20B482D-7A4B-420E-BEFA-D4B2B5C94752/Telegram.app/Telegram
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Telegram/Telegram-Structs.h>
#import <UIKit/UIButton.h>

@interface TGPhotoPaintSettingsWrapperView : UIButton {

	/*^block*/id _pressed;
	/*^block*/id _suppressTouchAtPoint;

}

@property (nonatomic,copy) id pressed;                           //@synthesize pressed=_pressed - In the implementation block
@property (nonatomic,copy) id suppressTouchAtPoint;              //@synthesize suppressTouchAtPoint=_suppressTouchAtPoint - In the implementation block
-(void)setSuppressTouchAtPoint:(id)arg1 ;
-(id)suppressTouchAtPoint;
-(id)hitTest:(CGPoint)arg1 withEvent:(id)arg2 ;
-(void)setPressed:(id)arg1 ;
-(id)pressed;
@end


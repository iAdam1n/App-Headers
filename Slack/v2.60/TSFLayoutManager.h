/*
* This header is generated by classdump-dyld 0.7
* on Friday, November 13, 2015 at 2:24:40 AM Greenwich Mean Time
* Operating System: Version 9.0.2 (Build 13A452)
* Image Source: /private/var/mobile/Containers/Bundle/Application/8DFB78BE-38F8-46CD-A2D2-C1EE6F510A67/Slack.app/Slack
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2014 by Elias Limneos.
*/

#import <Slack/Slack-Structs.h>
#import <UIFoundation/NSLayoutManager.h>

@interface TSFLayoutManager : NSLayoutManager
-(void)drawBackgroundForCharacterRange:(NSRange)arg1 atPoint:(CGPoint)arg2 ;
-(void)drawGlyphsForCharacterRange:(NSRange)arg1 atPoint:(CGPoint)arg2 ;
-(CGRect)frameRectForCharacterRange:(NSRange)arg1 ;
-(void)drawGlyphsForGlyphRange:(NSRange)arg1 atPoint:(CGPoint)arg2 ;
-(void)drawBackgroundForGlyphRange:(NSRange)arg1 atPoint:(CGPoint)arg2 ;
@end

